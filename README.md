...
- `get_pipeline_step_log(project_id, run_id, job_id, mode="around_failure", max_lines=200)` — the explicit, bounded follow-up for a single failing job named by `get_pipeline_run` (addressed by its `job_id`), for when that tool's compact `log_excerpt` isn't enough. Never returns the full/unbounded log — always slices it down to at most `max_lines` (hard-capped at 1000 regardless of what's requested). `mode` picks the slicing strategy: `"tail"` (last `max_lines` lines), `"around_failure"` (default; a `max_lines`-line window centered on the first error-looking line, falling back to `"tail"` behavior — reported as `mode: "around_failure->tail"` — when no such line is found), or `"errors_only"` (only the lines matching the error-pattern set, capped at `max_lines` matches). Returns `lines`, the effective `mode`, `truncated`, `total_lines`, `returned_lines`, and `more_available`. 404s are rewrapped with project/run/job context, same as `get_pipeline_run`.
