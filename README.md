...
- `get_pipeline_run(project_id, run_id, include_failure_excerpt=true, include_annotations=true)` — returns the run plus, for failed completed runs, a `failure` block listing each failing job (`name`, `url`, `failed_step`, and a `log_excerpt` of ~30 lines around the first `Error` / `FAILED` / `##[error]` marker). When `include_annotations=true` (default), each job also carries a normalized `annotations` list (`step`, `message`, `file`, `line`, `severity`, `title`) built from GitHub check-run / Azure Pipelines timeline annotations (GitLab has no structured surface, so its `annotations` is always `[]`). When `include_annotations=false`, the list is dropped in favor of a compact `annotation_count` + `annotations_fetched=false` pair, leaving `name`/`url`/`failed_step`/`log_excerpt` unaffected. 403/404 on the log endpoint degrades to `log_excerpt=null` and `note="logs unavailable"`. In-progress runs skip the failure fetch entirely.
