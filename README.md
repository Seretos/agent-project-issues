...
Read-only CI-run lookup against GitHub Actions. All three tools are token-gated only — no permission flag is required, mirroring `list_tickets` / `list_prs`. Works against `_auto`-discovered projects too.
