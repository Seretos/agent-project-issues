"""Regression test for ticket #304: bump the `lib-python-projects` exact-tag
pin in `pyproject.toml` from v0.3.15 to v0.3.16.
...
convention of #246/#254/#259/#270/#289 (assert the declared tag is >= this
ticket's floor) rather than #299's one-off exact-equality assertion, so this
test survives future bumps instead of going red for the wrong reason.
"""
...
def _tag_from_url(entry: str) -> str:
    # entry looks like: "lib-python-projects @ git+https://.../lib-python-projects@v0.3.15"
    return entry.rsplit("@", 1)[-1].strip()


def _declared_tag() -> str:
    """The exact vX.Y.Z tag currently declared for lib-python-projects in
    pyproject.toml (e.g. "v0.3.16")."""
    entry = _lib_python_projects_entry()
    return _tag_from_url(entry)


def test_lib_python_projects_pin_meets_v0_3_16_floor() -> None:
    """Driving test (R1): the declared pin must be >= v0.3.16. RED against
    the unbumped pyproject.toml, which still declares v0.3.15."""
    tag = _declared_tag()
...
def test_pin_comment_matches_declared_tag_and_floor() -> None:
    ...
    floor. RED because the comment currently still says "(v0.3.15)", which
    fails the >= 0.3.16 floor."""
    ...
    assert all(Version(tag.lstrip("v")) >= _MIN_VERSION for tag in found_tags), (
        ...
        f"meet the v0.3.16 floor"
    )


def test_installed_lib_matches_declared_pin() -> None:
    """Driving test (R3): the environment this suite runs in must have
    EXACTLY the tag declared in pyproject.toml installed. RIGHT NOW
    (pre-edit), pyproject.toml still declares v0.3.15 and the installed lib
    is also 0.3.15, so this test is GREEN, not RED, at file-creation time --
    that is expected. Its real RED window is transient, during phase=implement,
    between editing pyproject.toml (declares v0.3.16) and re-running
    sync-libs.ps1 (installs v0.3.16)."""
