...
This plugin ships a `UserPromptSubmit` hook (`hooks/security_hint.mjs`) that periodically reminds the agent to surface these guidelines whenever a `.seretos/` directory is present in the project tree. The hint hook runs under Node (the Claude Code runtime), not Python — the core plugin binary needs no Python toolchain and every consumer already has Node. Set `PROJECT_ISSUES_SECURITY_HINT_DISABLED` to a truthy value (`1`/`true`/`yes`/`on`, case/whitespace-tolerant) to opt out of the hook entirely.
...
- **Package layout.** The installed package carries `.claude-plugin/plugin.json` (Claude Code) and `.codex-plugin/plugin.json` (Codex), which points via `"mcpServers": "./.mcp.json"` at the `project-issues` stdio server (started via the plugin-relative `./bin/project-issues`, with the provider token variables forwarded by name through `env_vars`), alongside `hooks/`, `skills/` and `bin/`. There is no root `plugin.json`/`mcp.json` - no host reads them. `.github/scripts/stage-plugin-payload.sh` assembles this payload for both the install ZIP and the `release` branch.
...
- `list_pr_files(project_id, pr_id, include_patch=False, patch_max_chars=None)` — read-only, needs no `pulls.*` permission (mirrors `list_prs` / `get_pr`). Enumerates the PR's changed files with the diff positions `add_pr_review_comment` accepts (`line_ranges`: `{side, start, end}` entries per file). `line_ranges` is `[]` when the provider supports positions but this file has none (e.g. binary), and `null` when the provider can't supply positions at all (Azure DevOps) — check the top-level `supports_line_ranges` flag. `patch` is omitted by default; pass `include_patch=True` (optionally with `patch_max_chars`) to include it.
...
CI-run lookup and dispatch against GitHub Actions. `list_pipeline_runs` / `get_pipeline_run` / `get_pipeline_step_log` / `get_ref` / `list_releases` are token-gated only — no permission flag is required, mirroring `list_tickets` / `list_prs`. `trigger_pipeline` is the one write op and is gated by `permissions.pipelines.trigger`. Works against `_auto`-discovered projects too.

- `list_pipeline_runs(project_id, branch?, tag?, commit_sha?, ticket_id?, status, limit, workflow?, event?, since?)` — exactly one addressing argument must be set. `branch` filters Actions runs by branch name; `tag` resolves the tag ref to a commit SHA first; `commit_sha` filters by `head_sha`; `ticket_id` walks the ticket's timeline + body for linked PRs / branch hints, resolves them to head_shas, and aggregates runs (deduped by run id, sorted by `created_at` desc, capped to `limit`). When a ticket has no linked PR / branch, returns `runs=[]` plus a `hint` asking the user for a branch or commit. `workflow` / `event` / `since` are additional filters — not addressing arguments, combine with any of the five modes above: `workflow` matches by name (a bare name and a `.yml`/`.yaml` filename are equivalent); `event` accepts the canonical vocabulary (`manual`/`workflow_dispatch`/`push`/`schedule`/`pull_request`/`api`) resolved to each provider's native string, or a provider-native string directly; `since` is an ISO-8601 timestamp — a malformed value returns a structured error rather than silently filtering every run out. Each `PipelineRun` carries `id`, `name`, `branch`, `head_sha`, `event`, `status`, `conclusion`, `url`, `created_at`, `updated_at`, `run_attempt`.
- `get_pipeline_run(project_id, run_id, include_failure_excerpt=true, include_annotations=true)` — returns the run plus, for runs that concluded as `failure` or `cancelled` (GitHub reports a job killed by its own `timeout-minutes` as `cancelled`), a `failure` block listing each failing job (`name`, `url`, `failed_step`, `job_id`, and a `log_excerpt` of ~30 lines around the first `Error` / `FAILED` / `##[error]` marker). Each failing job's `job_id` is the handle to pass to `get_pipeline_step_log` when the ~30-line excerpt isn't enough. When `include_annotations=true` (default), each job also carries a normalized `annotations` list (`step`, `message`, `file`, `line`, `severity`, `title`) built from GitHub check-run / Azure Pipelines timeline annotations (GitLab has no structured surface, so its `annotations` is always `[]`). When `include_annotations=false`, the list is dropped in favor of a compact `annotation_count` + `annotations_fetched=false` pair, leaving `name`/`url`/`failed_step`/`job_id`/`log_excerpt` unaffected. 403/404 on the log endpoint degrades to `log_excerpt=null` and `note="logs unavailable"`. In-progress runs (`conclusion=null`) skip the failure fetch entirely.
- `get_pipeline_step_log(project_id, run_id, job_id, mode="around_failure", max_lines=200, max_chars=null)` — the explicit, bounded follow-up for a single failing job named by `get_pipeline_run` (addressed by its `job_id`), for when that tool's compact `log_excerpt` isn't enough. Never returns the full/unbounded log — always slices it down to at most `max_lines` (hard-capped at 1000 regardless of what's requested) AND at most 40,000 characters (hard-capped regardless of what's requested; a real CI log can pack far more than 40k characters into 1000 lines when lines are long, so the line cap alone isn't a safe bound). `max_chars` can only tighten that 40,000-character cap further, never loosen it. `mode` picks the slicing strategy: `"tail"` (last `max_lines` lines), `"around_failure"` (default; a `max_lines`-line window centered on the first error-looking line, falling back to `"tail"` behavior — reported as `mode: "around_failure->tail"` — when no such line is found), or `"errors_only"` (only the lines matching the error-pattern set, capped at `max_lines` matches). Character trimming (when it kicks in) preserves each mode's anchor: `"tail"` keeps the tail of the log, `"errors_only"` keeps the head of each retained matching line, `"around_failure"` keeps content closest to the matched error line. Returns `lines`, the effective `mode`, `truncated`, `total_lines`, `returned_lines`, and `more_available` — `truncated`/`more_available` cover both the line-based and character-based bounds, there is no separate flag for each. 404s are rewrapped with project/run/job context, same as `get_pipeline_run`.
