 ...
 *
 * Opt-out: set PROJECT_ISSUES_SECURITY_HINT_DISABLED to a truthy value
 * ("1"/"true"/"yes"/"on", case/whitespace-tolerant — mirrors the
 * PROJECT_ISSUES_DEBUG convention in tools/projects.py) to suppress the
 * hook entirely. This is checked as the very first thing in main(), before
 * stdin is even read, so an opted-out run touches no tmpdir state file and
 * never advances the rate-limit counter.
 *
 ...
 *   PROJECT_ISSUES_SECURITY_HINT_DISABLED=1 node -e '...' (or set the var
 *     in the environment) then pipe the same first-call JSON above in
 *     -> exit 0, no output, no state file written
 */
...
// Env var that opts out of this hook entirely. Truthy values disable it;
// anything else (unset, "0", "false", "", ...) leaves it enabled. Mirrors
// the PROJECT_ISSUES_DEBUG truthy-parsing convention in tools/projects.py
// ({"1", "true", "yes", "on"}, case/whitespace-tolerant).
const DISABLE_ENV = "PROJECT_ISSUES_SECURITY_HINT_DISABLED";
...
function main() {
  // Ticket #245: checked first, before stdin is even read and before the
  // counter/state-file is loaded or persisted — an opted-out run must
  // leave zero trace (no tmpdir state file, no counter advance).
  if (isDisabled()) {
    process.exit(0);
  }
