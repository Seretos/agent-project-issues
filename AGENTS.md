...
pinned to the exact immutable tag `@v0.1.13`.
...
- Installing test deps (`pip install -e ".[test]"`) pulls `lib-python-config` from GitHub
  (`@release/0.x`) and `lib-python-projects` at the pinned tag (`@v0.1.13`), so it needs network
  + git access.
...
- **`lib-python-projects` is pinned to the exact tag `v0.1.13` — no drift possible.** The tag is
