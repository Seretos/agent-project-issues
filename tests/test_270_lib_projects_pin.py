...
def test_pin_comment_names_the_new_tag() -> None:
    ...
    must name the new tag, not the old one. RED because line 22 currently
    still says "(v0.3.12)"."""
    ...
    # line itself), so "v0.3.13" must appear in the comment's own
    # parenthetical -- not merely on the dependency line via _declared_tag().
    comment_block = "\n".join(lines[comment_idx:dep_line_idx])

    assert "v0.3.13" in comment_block, (
        f"expected the lib-python-projects comment to mention v0.3.13, "
        f"got comment block:\n{comment_block!r}"
    )
