...
- **Package layout.** The installed package carries `.claude-plugin/plugin.json` (Claude Code) and `.codex-plugin/plugin.json` (Codex), which points via `"mcpServers": "./.mcp.json"` at the `project-issues` stdio server (started via the plugin-relative `./bin/project-issues`), alongside `hooks/`, `skills/` and `bin/`. There is no root `plugin.json`/`mcp.json` - no host reads them. `.github/scripts/stage-plugin-payload.sh` assembles this payload for both the install ZIP and the `release` branch.
