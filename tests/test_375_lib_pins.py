"""Regression test for work package #375: bump `lib-python-projects`
v0.3.23 -> v0.3.24 (`lib-python-config` stays at whatever v0.3.24 itself
...
Floor-based convention (like #246/#254/.../#308/#319/#366): the *declared*
tag must be an exact `vX.Y.Z` tag that is >= this ticket's floor (a floor,
not equality, so the tests survive future bumps). The pin-parsing helpers
below are copied from `tests/test_366_lib_pins.py` (itself copied from
`tests/test_319_lib_pins.py`), per the plan's instruction to follow the
per-bump floor convention and not edit an earlier ticket's floor test,
which would erase that ticket's record.

R1 (#375) projects pin bumped -- driving-test:
  - `test_projects_pin_meets_375_floor`: RED today -- declared and
    installed `0.3.23 < 0.3.24`.

R2 (config pin still matches what v0.3.24 requires) is `existing-suite` --
...
R5 (clean install/build/smoke stay green) is `ci-evidence` -- the
...
_PROJECTS_FLOOR = Version("0.3.24")
...
def test_projects_pin_meets_375_floor() -> None:
    """Driving test (R1). RED today: pyproject.toml declares
    lib-python-projects @v0.3.23, which is below this ticket's v0.3.24
    floor, and the installed version is 0.3.23 too."""
