"""Regression test for ticket #254: bump the `lib-python-projects` exact-tag
pin in `pyproject.toml` from v0.3.10 to v0.3.11 (tag confirmed real via
`git ls-remote --tags`).

Modelled on tests/test_246_lib_projects_pin.py's tomllib + packaging.requirements
...
def _declared_tag() -> str:
    ...
    pyproject.toml (e.g. "v0.3.11")."""
    entry = _lib_python_projects_entry()
    return _tag_from_url(entry)


def test_lib_python_projects_pin_meets_v0_3_11_floor() -> None:
    """Floor guard, not an exact-pin check: this test intentionally stays
    passing across future chore-ticket bumps (e.g. v0.3.12+) with zero edits.
...
def test_pin_comment_matches_declared_tag() -> None:
    ...
    (v0.3.11)."). Every vX.Y.Z-shaped token anywhere in the comment/dependency
