"""Regression tests for work package #398 (children #396/#392): bump BOTH
libs together, `lib-python-config` v0.1.3 -> v0.1.4 and `lib-python-projects`
v0.3.26 -> v0.4.0 (v0.4.0 requires config exactly v0.1.4, so either pin alone
fails to resolve).

Floor-based convention (like #246/#254/.../#308/#319/#366/#375/#385): the
...
The pin-parsing helpers below are copied from `tests/test_385_lib_pins.py`
(itself copied from `tests/test_375_lib_pins.py`, from `tests/
test_366_lib_pins.py`, from `tests/test_319_lib_pins.py`), per the plan's
instruction to follow the per-bump floor convention and not edit an earlier
ticket's floor test, which would erase that ticket's record. The duplicated
helpers are a known accepted cost (see the test_366 docstring).

R1 (#398) both pins bumped together -- driving-test:
  - `test_config_pin_meets_398_floor` / `test_projects_pin_meets_398_floor`:
    RED today -- declared `0.1.3 < 0.1.4` and `0.3.26 < 0.4.0`, and the
    installed versions are 0.1.3 / 0.3.26 too.

R2 (`(vX.Y.Z)` comments match the new pins) is `existing-suite` --
`test_319_lib_pins.py::test_pin_comment_names_only_its_own_declared_tag`
reads the declared tag dynamically, so it fails if a comment is left stale.

R3 (the two pins are mutually consistent: projects v0.4.0 requires config
exactly v0.1.4) is `existing-suite` --
`test_366_lib_pins.py::test_installed_projects_requires_pinned_config_tag`
reads the installed lib's own `Requires-Dist`.

R4 (no API break for the plugin) is `existing-suite` -- the full
`python -m pytest` suite, run via `pwsh scripts/test.ps1`.

R5 (clean install, PyInstaller build and smoke on windows-latest +
ubuntu-22.04) is `ci-evidence` -- the `.github/workflows/test.yml` PR run's
`test`, `build` and `smoke` jobs, not a test in this file.

Phase = tests: only the RED driving tests + scaffolding here. No production
...
_CONFIG_FLOOR = Version("0.1.4")
_PROJECTS_FLOOR = Version("0.4.0")
...
# ===========================================================================
# R1 -- both pins bumped together to meet the #398 floors
# ===========================================================================


def test_config_pin_meets_398_floor() -> None:
    """Driving test. RED today: pyproject.toml declares lib-python-config
    @v0.1.3, which is below this ticket's v0.1.4 floor, and the installed
    version is 0.1.3 too."""
    assert _exact_version("lib-python-config") >= _CONFIG_FLOOR
    assert _installed_version("lib-python-config") >= _CONFIG_FLOOR


def test_projects_pin_meets_398_floor() -> None:
    """Driving test. RED today: pyproject.toml declares lib-python-projects
    @v0.3.26, which is below this ticket's v0.4.0 floor, and the installed
    version is 0.3.26 too."""
