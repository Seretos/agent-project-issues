"""Regression test for ticket #328 (chore #327): bump the `lib-python-projects`
exact-tag pin in `pyproject.toml` from v0.3.19 to v0.3.20 (the release that
fixes the stale ETag-cache Link replay behind `list_comments(order="desc")`).

Floor-based, modelled on tests/test_308_lib_projects_pin.py. The generic
...
def test_lib_python_projects_pin_meets_v0_3_20_floor() -> None:
    """Driving test (R3): declared pin must be >= v0.3.20. RED while
    pyproject.toml still declares v0.3.19."""
    assert Version(_declared_tag().lstrip("v")) >= _MIN_VERSION


def test_pin_comment_names_a_tag_meeting_the_floor() -> None:
    """Driving test (R3): the comment block above the dependency names the
    declared tag, and that tag meets the floor. RED while it says v0.3.19."""
