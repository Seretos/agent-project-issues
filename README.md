...
- **Package layout (portable Agent Plugins 1.0).** The installed package has a root `plugin.json` (metadata) and `mcp.json` (the `project-issues` stdio server, started via the plugin-relative `./bin/project-issues`), alongside `.claude-plugin/` (Claude Code), `hooks/`, `skills/` and `bin/`. `.github/scripts/stage-plugin-payload.sh` assembles this payload for both the install ZIP and the `release` branch.
